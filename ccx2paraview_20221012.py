...
def get_inc_step(line):
  ...
  100CL  101 0.36028E+01         320                     3    1           1
  100CL  101 1.000000000         803                     0    1           1
  100CL  101 1.000000000          32                     0    1           1
  100CL  102 117547.9305          90                     2    2MODAL      1
  100CL  101 1.000000000       14836                     0    1           1
  100CL  101 1.000000000         116                     0    1           1
    """
    # line = line[12:]
    ...
    inc = float(line[13:24])
    step = int(line[58:63])
    # logging.debug('get_inc_step: inc={}, step={}'.format(inc, step))
    return inc, step
...
class NodalPointCoordinateBlock:
 ...
 -1      7419 8.28240E+02-1.52455E+03 5.00000E-01
 -1      7421 8.28240E+02-1.52455E+03-5.00000E-01
 -1      7422-8.28240E+02-1.52455E+03 5.00000E-01
        """
        ...
        while True:
            ...
            node_coords = [ float(line[13:25]),
                            float(line[25:37]),
                            float(line[37:49]), ]
            # logging.debug('{}\t{}\t{}\t{}'.format(node_number, *node_coords))
...
class ElementDefinitionBlock:
 ...
 -1       832    1    0    1
 -2      1657      1702      1697      1672      1658      1703      1698      1673
 -1         1    1    0AIR
 -2         1         2         3         4         5         6         7         8
 -1         1   10    0    1
 -2         1         2         3         4         5         6         7         8
 ...
 -2        10        12        11
        ...
        assert len(element_nodes) > 2, 'len(element_nodes) = {}'.format(len(element_nodes))
...
class NodalResultsBlock:
    ...
    One instance keeps data for one field variable in one step.
    """
 ...
 -4  DOR1  Rx    4    1
        ...
        self.name = line[5:17].rstrip() # dataset name
        # self.name = match.group(1) # dataset name
        # logging.debug('read_components_info: ' + self.name)
 ...
 -5  SZX         1    4    3    1
        ...
        while True:
            ...
            component_name = line[5:17].rstrip()
            # logging.debug('read_components_info: ' + component_name)
    ...
    def set_point_components(self, point_num, components):
        """point_num is a renumbered node number.
        VTK point numbering starts from 0."""
        self.set_component_names()
        self.data_array.InsertTuple(point_num, components)
 ...
 -2           5.31719E+01 6.69780E+01 2.76244E+01 2.47686E+01 1.99930E+02 2.14517E+02
        ...
        for point_num in renumbered_nodes.values():
            self.set_point_components(point_num, [0]*self.ncomps)
        ...
        while True:
            ...
            # if self.name == 'U':
            # assert len(components) == 3, 'len(components) == {}'.format(len(components))

            # logging.debug('read_nodal_results: {} {}'.format(node_num, components))
            if node_num in renumbered_nodes:
                point_num = renumbered_nodes[node_num]
            else:
                point_num = self.data_array.GetNumberOfTuples()
            self.set_point_components(point_num, components)
...
class Converter:
        ...
        if step:
            while True:
                ...
                if key == '  100':
                    ...
                    assert b.data_array.GetNumberOfTuples() == b.node_block.numnod, 'len(values) != len(nodes)'
    ...
    def calculate_mises(self, b):
        ...
        for point_num in renumbered_nodes.values():
            ...
            b1.set_point_components(point_num, [mises])
    ...
    def calculate_principal(self, b):
        ...
        for point_num in renumbered_nodes.values():
            ...
            b1.set_point_components(point_num, sorted(eigenvalues))
